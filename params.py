...
ninv_t_Path = os.path.join(ALILENS, 'sims/ninv/ninv_t1.fits')
ninv_p_Path = os.path.join(ALILENS, 'sims/ninv/ninv_p1.fits')
...
ss_dict = { k : v for k, v in zip( np.arange(nsims), np.concatenate([np.roll(range(i*22, (i+1)*22), -1) for i in range(0,9)]))}
...
mc_sims_bias = np.arange(44)
mc_sims_var  = np.arange(44, 198)
