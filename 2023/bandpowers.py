...
class ffp10_binner:
    ...
    def get_bmmc(self, mc_sims_dd=None, mc_sims_ss=None):
        ...
        # NOTE N1 calculation can not be parallal so for speed we can have @Jinyi
    ...
    def get_bmmc_cov(self, mc_sims_dd=None):
        """Covariance matrix obtained from the multiplicative MC correction.

        """
        return np.outer(np.sqrt(np.absolute(self.get_bmmc())), np.sqrt(np.absolute(self.get_bmmc())))

    def get_cov(self):
        """Covariance matrix obtained from MCN0 corrected by mMC.

        """
        return self.get_mcn0_cov() * self.get_bmmc_cov()
